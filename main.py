...
def compute_late_minutes(dt: datetime, shift: str) -> int:
    ref = dt.replace(hour=8, minute=30, second=0, microsecond=0) if shift=="Morning" else dt.replace(hour=17, minute=0, second=0, microsecond=0)
    delta = dt - ref
...
async def handle_clock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    confirm = await context.bot.send_message(
        msg.chat_id,
        f"✅ [{escape_markdown(full_name)}](tg://user?id={user.id}) clocked in at `{clock_in_str}` ({shift})",
        parse_mode=ParseMode.MARKDOWN
    )
    link = f"https://t.me/c/{str(GROUP_ID)[4:]}/{confirm.message_id}"
    caption = (
        "#clock\n"
...
async def cmd_sick_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # mark full day even after clock-in
    cur.execute("""
        INSERT OR REPLACE INTO attendance (user_id, full_name, date, clock_in, clock_out, status, late_minutes)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur = conn.cursor()
    cur.execute("""
        SELECT full_name, user_id, clock_in, status
        FROM attendance
        WHERE date=?
        ORDER BY clock_in
    """, (today,))
    ...
    conn.close()
    lines=[]
    for full_name, uid, clock_in, status in rows:
        if clock_in:
            lines.append(f"• **[{escape_markdown(full_name)}](tg://user?id={uid})** - `{clock_in}`")
        else:
            lines.append(f"• **[{escape_markdown(full_name)}](tg://user?id={uid})** - {status}")
...
async def handle_glass_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if not msg or not msg.photo or msg.chat_id != GROUP_ID:
        return
...
async def cmd_total(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*), COUNT(DISTINCT reported_by_id) FROM glass_logs WHERE date LIKE ? AND group_id=?", (f"{month_prefix}%", GROUP_ID))
    total_broken, reporter_count = cur.fetchone()
    cur.execute("SELECT reported_by_name, COUNT(*) as c FROM glass_logs WHERE date LIKE ? AND group_id=? GROUP BY reported_by_id, reported_by_name ORDER BY c DESC", (f"{month_prefix}%", GROUP_ID))
    rows = cur.fetchall()
    conn.close()
...
async def cmd_report_attendance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    conn = sqlite3.connect(DB_FILE)
    df = pd.read_sql_query("SELECT a.date, a.full_name, a.status, a.clock_in, a.clock_out, a.late_minutes, a.user_id FROM attendance a LEFT JOIN staff s ON a.user_id = s.user_id", conn)
    conn.close()
    if df.empty:
...
async def cmd_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    df_glass = pd.read_sql_query("SELECT * FROM glass_logs", conn)
    conn.close()
    if df_attendance.empty and df_glass.empty:
    ...
    with pd.ExcelWriter(bio, engine='xlsxwriter') as writer:
        ...
        df_glass.to_excel(writer, sheet_name='GlassLogs', index=False)
    bio.seek(0)
